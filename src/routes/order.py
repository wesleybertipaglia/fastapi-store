...
from src.schemes.order import Order, OrderCreate, OrderUpdate, OrderList
...
def list_orders(db: Session = Depends(get_db), current_user_id: str = Depends(AuthRepository(db).get_current_user_id), skip: int = 0, limit: int = 10):
    return OrderRepository(db).list_orders(user_id=current_user_id, skip=skip, limit=limit)
