...
from src.schemes.payment import PaymentMethod, PaymentMethodUpdate, PaymentMethodList, PaymentMethodGet
...
@router.get('/{id}', response_model=PaymentMethodGet)
def get(id: str, db: Session = Depends(get_db)):
    return PaymentMethodRepository(db).get(id)

@router.post('/', status_code=status.HTTP_201_CREATED, response_model=PaymentMethodGet)
def create(payment_method: PaymentMethod, db: Session = Depends(get_db), current_user_id: str = Depends(AuthRepository(db).get_current_user_id)):
    return PaymentMethodRepository(db).create(user_id=current_user_id, payment_method=payment_method)

@router.put('/{id}', response_model=PaymentMethodGet)
