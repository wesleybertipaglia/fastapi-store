...
from src.schemes.user import UserSingle
...
@router.post('/up', status_code=status.HTTP_201_CREATED, response_model=UserSingle)
...
async def delete(user: Delete, current_user_id: str = Depends(AuthRepository(db).get_current_user_id)):
    return UserRepository(db).delete(current_user_id, user.password)